...
data1 = render_data.read_file(data_src_dir + signal_path + str(signal_id) + ".txt", array_size=1024)
all_data = render_data.read_files_in_dir(data_src_dir + signal_path, array_size=1024, num_of_files=100)
data2 = render_data.read_files_in_dir(data_src_dir + path_minus_0_5, array_size=1024, num_of_files=100)
data3 = render_data.read_files_in_dir(data_src_dir + path_minus_0_25, array_size=1024, num_of_files=100)
data4 = render_data.read_files_in_dir(data_src_dir + path_zero, array_size=1024, num_of_files=100)
data5 = render_data.read_files_in_dir(data_src_dir + path_plus_0_25, array_size=1024, num_of_files=100)
data6 = render_data.read_files_in_dir(data_src_dir + path_plus_0_5, array_size=1024, num_of_files=100)

data1 = np.mean(data1, axis=1)
all_data = render_data.average_data(data=all_data)
data2 = render_data.average_data(data=data2)
data3 = render_data.average_data(data=data3)
data4 = render_data.average_data(data=data4)
data5 = render_data.average_data(data=data5)
data6 = render_data.average_data(data=data6)

fig, ax = render_data.draw_amplitudes(data1, color1)
render_data.draw_amplitudes(data2, color2, fig=fig, ax=ax)
render_data.draw_amplitudes(data3, color3, fig=fig, ax=ax)
render_data.draw_amplitudes(data4, color4, fig=fig, ax=ax)
render_data.draw_amplitudes(data5, color5, fig=fig, ax=ax)
render_data.draw_amplitudes(data6, color6, fig=fig, ax=ax)

fig.savefig(path + "AmplitudeValues" + ".pdf")

render_data.draw_amplitudes(data1, color1, fig=fig, ax=ax)
...
fig, ax = render_data.draw_amplitudes(sin_data, color1)
...
render_data.draw_arcsin_lines(a1, b1, a2, b2, ax, 50)

fig.savefig(path + "ArcsinParams" + ".pdf")

data1 = render_data.set_amplitude(data1, ampl)
all_data = render_data.set_amplitude(all_data, ampl)

fig, ax = plt.subplots()
x_i = render_data.draw_delta_times(data=data1, num_of_dots=40, ax=ax)
fig.savefig(path + "TimePeriods" + ".pdf")

fig, ax = plt.subplots()
render_data.draw_hist(data=all_data, ax=ax)
